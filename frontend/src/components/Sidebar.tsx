...
import { Database, GitBranch, FolderOpen, AlertCircle, Plus, X, RotateCcw, ChevronLeft, ChevronRight } from 'lucide-react'
...
export default function Sidebar({ status, progress, onNeedOnboarding: _onNeedOnboarding, onGraphReload }: Props) {
  ...
  return (
    ...
    >
      ...
      <button
        ...
        className="absolute -right-3 top-16 z-30 w-6 h-6 rounded-full glass shadow-sm border border-bina-border flex items-center justify-center text-bina-muted hover:text-bina-accent transition-colors no-drag"
      >
        {collapsed
      ...
      <div className={`h-14 drag-region overflow-hidden flex ${collapsed ? 'items-center justify-center' : 'items-end px-3 pb-3'}`}>
        {activeWorkspace ? (
          <div className="flex items-center gap-2 no-drag min-w-0">
            <span className="text-[20px] leading-none flex-shrink-0">{activeWorkspace.emoji}</span>
            {!collapsed && (
      ...
      <div className="flex-1 px-2 py-4 space-y-1 overflow-y-auto overflow-x-hidden" style={{ minWidth: 0 }}>
        {/* Indexing progress */}
        {isIndexing && !collapsed && (
          <div className="bg-bina-accent/10 border border-bina-accent/20 rounded-xl p-3 mb-3">
            <div className="flex items-center gap-2 mb-2">
              <div className="w-2 h-2 rounded-full bg-bina-accent animate-pulse flex-shrink-0" />
        ...
        {[
          ...
          hide ? null : (
            <div
              key={label}
              className={`rounded-xl hover:bg-bina-accent/5 transition-colors ${collapsed ? 'px-0 py-2 flex justify-center' : 'px-2 py-2'}`}
              title={collapsed ? `${label}: ${value}` : undefined}
            >
              {collapsed ? (
                <Icon className={`w-[20px] h-[20px] ${color} flex-shrink-0`} />
              ) : (
          ...
          {collapsed && (
            <div className="flex flex-col items-center gap-2 py-1">
              <button
                ...
                disabled={addingFolder || !activeWorkspaceId}
                title="Add folder"
                className="w-[22px] h-[22px] rounded flex items-center justify-center text-bina-muted hover:text-bina-accent hover:bg-bina-accent/10 transition-colors disabled:opacity-40"
              >
          ...
          {folders.map((f) => (
            collapsed ? (
              <div
                ...
                title={f.folder_path.split('/').pop() || f.folder_path}
              >
                <FolderOpen className="w-[20px] h-[20px] text-bina-yellow flex-shrink-0" />
              </div>
            ) : (
              <div
                key={f.folder_path}
                className="flex items-start gap-2 px-2 py-2 rounded-xl hover:bg-bina-accent/5 transition-colors group"
              >
                <FolderOpen className="w-[20px] h-[20px] text-bina-yellow flex-shrink-0 mt-0.5" />
                <div className="flex-1 min-w-0">
